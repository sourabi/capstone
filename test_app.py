...
class CapstoneTestCase(unittest.TestCase):
    ...
    def test_patch_movies(self):
        res = self.client().patch('/movies/7', headers=self.casting_director, json={"title": "insurgent"})
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertTrue(data['movies'])
    ...
    def test_failed_patch_movies_404(self):
        res = self.client().patch('/movies/1', headers=self.executive_producer, json={"title": "insurgent"})
        data = json.loads(res.data)
    ...
    def test_patch_actors(self):
        res = self.client().patch('/actors/7', headers=self.casting_director, json={"name": "cox"})
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertTrue(data['actors'])
    ...
    def test_failed_patch_actors_403(self):
        res = self.client().patch('/actors/7', headers=self.casting_assistant, json={"name": "cox"})
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 403)
        self.assertEqual(data['code'], 'unauthorized')
        self.assertEqual(data['description'], 'Permission not found.')
